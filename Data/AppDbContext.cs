...
namespace f1api.Data
{
    public class AppDbContext(DbContextOptions<AppDbContext> options) :DbContext(options)
    {
        ...
        public DbSet<UserDriverVote> UserDriverVotes => Set<UserDriverVote>();
        public DbSet<UserTeamVote> UserTeamVotes => Set<UserTeamVote>();
        public DbSet<UserNftReward> UserNftRewards => Set<UserNftReward>();
        public DbSet<RacePrediction> RacePredictions => Set<RacePrediction>();
        public DbSet<DriverFanRating> DriverFanRatings => Set<DriverFanRating>();
        public DbSet<TeamFanRating> TeamFanRatings => Set<TeamFanRating>();
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<UserDriverVote>()
        .HasOne(v => v.FirstDriver)
        .WithMany()
        .HasForeignKey(v => v.FirstDriverId)
        .OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<UserDriverVote>()
                .HasOne(v => v.SecondDriver)
                .WithMany()
                .HasForeignKey(v => v.SecondDriverId)
                .OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<UserDriverVote>()
                .HasOne(v => v.ThirdDriver)
                .WithMany()
                .HasForeignKey(v => v.ThirdDriverId)
                .OnDelete(DeleteBehavior.Restrict);

            // Aynı ayarı RacePrediction için de yapmalısın
            modelBuilder.Entity<RacePrediction>()
                .HasOne(r => r.Winner)
                .WithMany()
                .HasForeignKey(r => r.WinnerId)
                .OnDelete(DeleteBehavior.Restrict);
